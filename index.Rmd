...
season_num_plot <- ggplot(count_season, aes(x=season, y=n)) +
    geom_line(aes(y = n)) +
...
season_ind_count <- season_j_ind %>%
  select(season, deploy_off_type) %>%
  group_by(season, deploy_off_type) %>%
  count() 
...
# season 2 ...
# season 1...
b <- ggplot(season_ind_count, aes(fill=deploy_off_type, y=n, x=season)) + 
