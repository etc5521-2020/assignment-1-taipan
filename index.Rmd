...
```{r deploy-off-type, fig.cap="Distribution of the classification of tag deployment end"}
...
It can be seen from figure \@ref(fig:deploy-off-type) that out of all the deploy_off_type stated in the description, there are only four types available in the dataset, which are dead, removal, other and unknown. The highest number of ... is unknown, which indicates that there was no proper tracking and documentation. 

As there are several type of deads in the data, we are looking at the cause of dead of caribou in more details. 
...
The highest number of cause of death is predation by wolf, which is almost ..% of the total cause of death, which also includes the suspected wolf predation ones. The other big part of the cause of death includes predation by bear or predation by unknown animals, which concludes that the majority of the death of the caribou was due to predation.
The other causes such as vehicle collision, train collision, and accidentals are only forming (?) a small part of the death causes.
However, there is also a big amount of unknown which indicates that the (pengawasan?) isnt that good.
...
and then create a graph of the number of caribou overtime and put in different lines for different caribou type(the study site) and also make one which is the overall.

so can see which caribou type is the most increasing one
...
joined with the individuals dataset to add study_site

limitations though cus there might be some caribou that were not tracked in certain years.
as we are calculating the number of caribou per year by using the documented locations data, it might not be an accurate analysis as there might be some caribou that were not tracked in certain years, or that the locations were not being documented inside the locations dataset.

Conc: in need of better tracking and documentation.
