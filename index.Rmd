...
ggplotly(season_site_plot)
```

```{r}
library(hrbrthemes)
season_num <- fourseason %>%
  group_by(year, season) %>%
  summarise(animals = unique(animal_id, .groups = 'drop')) %>%
  group_by(animals) %>%
  summarise_all(n_distinct) %>%
  mutate(`Total season number` = year*season) 

count_season <- season_num %>%
  count(`Total season number`)

season_num_plot <- ggplot(count_season, aes(x=`Total season number`, y=n)) +
    geom_line(aes(y = n)) +
  ...
  xlab("Tracking duration (number of season)") +
  ylab("Number of Caribou") +
  ggtitle("Number of Caribou") 
ggplotly(season_num_plot)
```

```{r}
library(ggridges)
i_keep_duplicate_last <- individuals %>%
  group_by(animal_id) %>%
  slice(n())

season_j_ind <- season_num %>%
  left_join(i_keep_duplicate_last, by = c("animals" = "animal_id"))

season_ind_count <- season_j_ind %>%
  select(`Total season number`, deploy_off_type) %>%
  group_by(`Total season number`, deploy_off_type) %>%
  count() 

# season 4 other, dead, removal, unknow how many percentage for each
# season 3 ...
# season 2 ...
# season 1...
b <- ggplot(season_ind_count, aes(fill=deploy_off_type, y=n, x = as.factor(`Total season number`))) + 
    geom_col(position="stack") +
    ggtitle("Number of Caribou by each of their duration of tracking") +
    xlab("Deployment season duration") +
    ylab("count") 
